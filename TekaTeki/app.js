var TekaTeki = (function () {
    ...
    TekaTeki.prototype.preload = function () {
    ...
    TekaTeki.prototype.create = function () {
        ...
        this.Timeout = 30000;
        this.SaveTime = 3000;
        this.CreditsText = this.game.add.text(this.game.width / 2, this.game.height - 3, "Developed in Surabaya for my nephew Na'il Ar Rasyid Hanifa.\nCredits to Phaser.io, Audacity, Inkscape, and LMMS.io. Mumbling_Man Games 2018.", this.TextStyle3);
            ...
            if (on) {
                FBInstant.getLeaderboardAsync('The Score Rank')
                    ...
                    for (var i = 0; i < entries.length; i++) {
                        _this.RankText.text += '(' + entries[i].getRank() + '.)\t' + entries[i].getPlayer().getName() + '\t\t' + entries[i].getScore() + '\n';
                    }
        ...
        this.showQuitDialog = function (on) {
            if (on) {
        ...
        this.showGameOverDialog = function (on) {
            if (on) {
                if (this.Score >= this.Highscore)
                    this.saveHighscore(true);
        ...
        this.loadHighscore = function () {
            ...
            FBInstant.getLeaderboardAsync('The Score Rank')
                ...
                .then(function (entry) {
                _this.Highscore = entry.getScore();
                _this.HighscoreText.text = _this.Highscore;
        ...
        this.saveHighscore = function (upload) {
            var _this = this;
            ...
            if (upload) {
                FBInstant.getLeaderboardAsync('The Score Rank')
                    ...
                    _this.Highscore = Math.round(_this.Highscore);
                    _this.HighscoreText.text = _this.Highscore.toString();
                    return leaderboard.setScoreAsync(_this.Highscore, '');
                })
                    .then(function () { return console.log('Score saved'); })
                    .catch(function (error) { return console.error(error); });
            }
        };
        this.clamp = function (input, max) {
            if (input > max)
                return max;
            else
                return input;
        ...
        for (var b = 1; b < 9; b++) {
            for (var a = 1; a < 7; a++) {
                var node = this.game.make.button(-(this.Board.width / 2) - (this.Board.width / 12) + (a * this.Board.width / 6), -(this.Board.height / 2) - (this.Board.height / 16) + (b * this.Board.height / 8), "NodeYellow");
    ...
    TekaTeki.prototype.update = function () {
        if (this.InMainMenu) {
    ...
    TekaTeki.prototype.postSplashEvent = function () {
    ...
    TekaTeki.prototype.buttonPlayEvent = function () {
        this.SfxIn.play();
        this.setupGameplay();
        this.showMainMenu(false);
        this.showGameplay(true);
    };
    TekaTeki.prototype.buttonRankEvent = function () {
        this.SfxIn.play();
        this.showMainMenu(false);
        this.showRank(true);
    };
    TekaTeki.prototype.buttonRulesEvent = function () {
        this.SfxIn.play();
        this.showMainMenu(false);
        this.showRules(true);
    };
    TekaTeki.prototype.buttonQuitRankEvent = function () {
        this.SfxOut.play();
        this.showRank(false);
        this.showMainMenu(true);
    };
    TekaTeki.prototype.buttonQuitRulesEvent = function () {
        this.SfxOut.play();
        this.showRules(false);
        this.showMainMenu(true);
    };
    TekaTeki.prototype.buttonQuitGameEvent = function () {
    ...
    TekaTeki.prototype.buttonYesEvent = function () {
        this.SfxIn.play();
        if (this.Timeout < 0) {
            this.showGameOverDialog(false);
            this.setupGameplay();
    ...
    TekaTeki.prototype.buttonNoEvent = function () {
        this.SfxOut.play();
        if (this.Timeout < 0) {
            this.showGameOverDialog(false);
            this.showGameplay(false);
    ...
    TekaTeki.prototype.nodeSelect = function (node) {
        if (this.IsReadyToPlay) {
            this.SfxIn.play();
